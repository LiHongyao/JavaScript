...
  事件一定要添加在某个页面元素后才会生效，包括window对象和document对象。事件的添加主要有四种方式：

## 1、通过页面标签元素添加

  在页面标签元素的 “***on-****”属性上添加，如例：

```html
<!-- HTML -->
<form>
	<input type="text" oninput="validate()">
	<button type="submit" onclick="formSubmit()">提交</button>
</form>
...
  这种方式和上面一种方式的形式一样，同样是对实例化的Element对象进行事件绑定，但这种绑定的方式一般叫做添加 “***监听函数*** ”，需要使用Element的***addEventListener()***方法，而且事件名称不需要前缀“*on*”。表示方法如下：
...
- ***useCapture***：表示监听函数是在 ***捕获阶段*** 触发还是在 ***冒泡阶段*** 触发，用一个布尔值表示。true表示在冒泡阶段触发，false（默认）表示在捕获阶段触发。老式浏览器规定该参数必写，较新版本的浏览器允许该参数可选。为了保持兼容，建议总是写上该参数。
...
  “***事件对象*** ” 指的是当一个事件产生后，会在事件函数内部生成一个“*Event*”的对象实例，并且将这个示例作为事件函数的参数（通常命名为“e”或“event”，IE下需要做兼容处理），我们尝试在一个事件中输出这个Event对象：
...
  由于事件传播的机制，会使得同一个事件也会在同一个元素上多次触发。比如，假设点击 *\<div>* 之中嵌套一个 *\<p>* 节点。
...
<div>
	<div id="div">
		<p>这里是p标签的内容，我的点击事件由父级元素&lt;div&gt;代理</p>
	</div>
</div>
...
<script type="text/javascript">
	...
	div.onclick = function(e) {
		var targetEle = e.target.tagName.toLowerCase();
		if (targetEle == 'p') {
			this.textContent += '!!!';
		}
	}
	// 点击元素输出：“这里是p标签的内容，我的点击事件由父级元素<div>代理 !!!”
</script>
```
