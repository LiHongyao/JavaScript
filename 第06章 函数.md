...
所谓 [函数](https://developer.mozilla.org/zh-CN/docs/Web/JavaScript/Guide/Functions)，就是使用关键字 `function` 定义的一段具有**独立作用域**，能被**反复执行**的语句块，或者说函数就是功能。函数在JavaScript中也是以一种“值”的方式存在的，和其它数据类型相比，它是一种可以接收参数的、可运行的值，除此之外，它并没有任何的不同。JavaScript作为一个“**面向对象**”和“**函数式编程**”的语言，因此函数部分自然是一个重点 
...
闭包是基于词法作用域书写代码时所产生的自然结果。JavaScript中 闭包无处不在，你只需要能够识别并拥抱它。简单理解：闭包就是能够读取其他函数内部变量的函数，使得函数不被GC（垃圾回收机制）回收。
