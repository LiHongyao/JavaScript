...
  如果正则模式是一个空字符串，则匹配所有字符串。字面量形式返回false。
...
```javascript
var str = "什么是老司机？谁是老司机？"

str.replace(/老司机/, '经验丰富的人')
// "什么是经验丰富的人？谁是老司机？"

str.replace(/老司机/g, '经验丰富的人')
// "什么是经验丰富的人？谁是经验丰富的人？"
```
...
// 提示：\w表示字符类（包括大小写字母，数字）；+表示至少一个，作用在前一个字符，即\w+；\s匹配任意的空白符；\S匹配任意不是空白符的字符
...
// 示例1
var str = "妖姬发条阿狸";
str.replace(/妖姬|发条|阿狸/g, function(match, idx, str){
     return '\n匹配值：' + match + '，匹配位置：' + idx + '，原字符串：' + str + '\n';
});
/*"
匹配值：妖姬，匹配位置：0，原字符串：妖姬发条阿狸

匹配值：发条，匹配位置：2，原字符串：妖姬发条阿狸

匹配值：阿狸，匹配位置：4，原字符串：妖姬发条阿狸
"*/

// 示例2
var str = "【妖姬发条阿狸】";
str.replace(/(妖姬)(发条)(阿狸)/, function(match, $1, $2, $3, idx, str){
     return '|' + $1 + $2 + $3 + '|'
});
// "【|妖姬发条阿狸|】"
```
